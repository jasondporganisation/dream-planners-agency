...
REQUEST_DELAY_S = 1.5  # polite gap between requests to the same host
...
class PoliteSession:
    ...
    def get(self, url: str, *, retries: int = 3, timeout: int = 45, **kw) -> requests.Response:
        last_err = None
        for attempt in range(1, retries + 1):
            self._wait()
            try:
                res = self.session.get(url, timeout=timeout, **kw)
